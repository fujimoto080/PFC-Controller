...
import { updateCloudSettings } from '@/lib/cloud-data';
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await updateCloudSettings(syncKey, body.settings as Record<string, unknown>, updatedAt);

    return NextResponse.json({ ok: true, updatedAt, mode: 'update' });
  } catch (error) {
    console.error('クラウド設定updateエラー', error);
    return NextResponse.json(
      { error: 'クラウド設定のupdateに失敗しました' },
      { status: 500 },
    );
  }
}
