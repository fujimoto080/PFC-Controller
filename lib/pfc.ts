...
function mapPFC(fn: (key: (typeof PFC_KEYS)[number]) => number): PFC {
  return { protein: fn('protein'), fat: fn('fat'), carbs: fn('carbs'), calories: fn('calories') };
}

/** PFC の合計を小数第2位で丸めて返す。 */
export function sumPFC(items: readonly PFC[]): PFC {
  return mapPFC((key) => roundPFC(items.reduce((acc, item) => acc + item[key], 0)));
}
...
export function activityAdjustedCalorieTarget(targetCalories: number, log: DailyLog): number {
  const burned = log.activities.reduce((total, activity) => total + activity.caloriesBurned, 0);
...
function addDays(date: string, days: number): string {
  return formatDate(new Date(`${date}T00:00:00+09:00`).getTime() + days * DAY_MS);
}
 ...
 */
export function computePfcDebt(currentDate: string, target: PFC, logs: Logs): PFC {
...
export function weeklyAverage(logs: Logs, today: string): PFC {
  const totals = Array.from({ length: 7 }, (_, i) => logs[addDays(today, -i)]?.total ?? EMPTY_PFC);
  return mapPFC((key) => roundPFC(totals.reduce((acc, t) => acc + t[key], 0) / 7));
}
