  ...
  return (
    <div
      className={cn(
        ...
        isFirst ? 'h-2' : 'mt-1 h-2 border border-red-500/30',
      )}
    >
      <motion.div
        initial={{ width: 0 }}
      ...
      <motion.div
        ...
        className={cn(
          'absolute top-1/2 h-[40%] -translate-y-1/2 rounded-full opacity-50',
          color,
        )}
      />
    </div>
...
export function DebtStackedBars({
  current,
  debt,
  target,
  color,
  delay = 0,
  maxBars = 10,
}: DebtStackedBarsProps) {
  const extraBarsCount = Math.ceil(
    Math.max(0, current + debt - target) / target,
  );
  ...
  return (
    <>
      ...
      {Array.from({ length: Math.min(maxBars, extraBarsCount) }).map((_, i) => {
        ...
        const currentInThisBar = Math.max(
          0,
          Math.min(safeTarget, current - barStart),
        );
        const debtStart = current;
        const debtEnd = current + debt;
        const debtInThisBar = Math.max(
          0,
          Math.min(barEnd, debtEnd) - Math.max(barStart, debtStart),
        );
